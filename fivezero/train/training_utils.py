    ...
    def sample_from_buffer(self, batch_size):
        ...
        for i in range(len(self.buffer)):
            if batch_sizes[i] > len(self.buffer[i]):
                ...
                output_epochs.extend(
                    [self.latest_epoch_index + (i - 2)] * len(self.buffer[i])
                )
            else:
                sample_indices = np.random.choice(
                    len(self.buffer[i]), size=batch_sizes[i], replace=False
                )
                output_sample.extend([self.buffer[i][j] for j in sample_indices])
                output_game_index.extend(
                    [self.game_index_buffer[i][j] for j in sample_indices]
                )
                output_epochs.extend(
                    [self.latest_epoch_index + (i - 2)] * batch_sizes[i]
                )
...
def node_to_child_distribution(parent, temperature) -> np.ndarray:
    ...
    assert (
        parent.fully_expanded()
    ), "node_to_child_distribution called on non-fully-expanded node"
    assert (
        len(parent.children) > 0
    ), "node_to_child_distribution called on node with no children"
...
def training_step(batch_traces, net, value_criterion, policy_criterion, optimizer):
    ...
    batch_states = torch.concatenate(
        [net.encode(state) for state in batch_states], dim=0
    )

    batch_moves = torch.tensor(
        [child_node.move for _, child_node, _ in batch_traces],
        dtype=torch.int64,
        device=device,
    )
    batch_zs = torch.tensor(
        [z * parent_node.actor for parent_node, _, z in batch_traces],
        dtype=torch.float32,
        device=device,
    )
    child_values = [
        parent_node.value_of_children(net) for parent_node, _, _ in batch_traces
    ]
    ...
    value_predictions = [value_predictions[i, 0] for i, _ in enumerate(batch_traces)]
    ...
    slope, intercept, r_value, p_value, std_err = linregress(
        child_values.detach().cpu().numpy().ravel(),
        policy_predictions.detach().cpu().numpy().ravel(),
    )
    ...
    empirical_policies = [
        node_to_child_distribution(parent_node, 1.0)
        for parent_node, _, _ in batch_traces
    ]
    ...
    assert torch.isfinite(
        empirical_policies
    ).all(), "Non-finite values in empirical policies"
    ...
    return (
        ...
        policy_loss.item(),
        loss.item(),
        policy_predictions.detach().cpu().numpy(),
        empirical_policies.detach().cpu().numpy(),
        value_predictions.detach().cpu().numpy(),
        batch_zs.detach().cpu().numpy(),
        slope,
    )
