...
from fivezero.gameEngine import Actor, State, legal_moves, new_game, step
...
class Node:
    ...
    def puct_selection_value(self):
        ...
        return -1 * self.value / (1 + self.visits) + c * (
            self._P + eta
        ) * np.sqrt(  # exploration bonus
            self.parent.visits
        ) / (
            1 + self.visits
        )
    ...
    def value_of_children(self, policy_net: ConvNet):
        ...
        for move in range(25):
            if move in legal_moves(self.game_state):
                values.append(
                    policy_net.forward_value(
                        policy_net.encode(step(self.game_state, move))
                    ).item()
                )
            else:
                values.append(0)
    ...
    def fully_expand(self, policy_net: ConvNet | None, overwrite: bool = False):
        if len(self.children) > 0 and not overwrite:
            raise ValueError(
                "Node already has children, set overwrite=True to overwrite"
            )
        ...
        for possible_move in legal_moves(self.game_state):
            new_child = Node(
                move=possible_move,
                actor=-self.actor,
                game_state=step(self.game_state, possible_move),
                parent=self,
            )
    ...
    def uct_select(self):
        ...
        for child in self.children:
            # need to flip the sign of the child value because it is from the perspective of the child
            selection_value = -1 * child.value / (1 + child.visits) + c * np.sqrt(
                self.visits
            ) / (1 + child.visits)
            if selection_value > previous_max:
                previous_max = selection_value
    ...
    def print_children(self):
        ...
        print(
            f"{'Move':<10} {'Visits':>8} {'Value':>8} {'Policy':>10} {'UCT Value':>10}"
        )
        print("-" * 40)
